...
from utils import numpy2tensor, normalize_batch, get_pos_object, \
                  MeanShift, Vgg16
################## Gradient Operator #########################
...
def gaussian_poisson_editing_loss(X, x_src, x_mask, param_l, param_g, color_weight=1, content_weight=1, eps=1e-12, gpu_id=0):
    Fh = (X[:, :, :, 1] + np.roll(X[:, :, :, 3], -1, axis=1)) / 2
    Fv = (X[:, :, :, 2] + np.roll(X[:, :, :, 4], -1, axis=0)) / 2
    L = np.roll(Fh, 1, axis=1) + np.roll(Fv, 1, axis=0) - Fh - Fv
    ...
    vgg = Vgg16().to(gpu_id)
    run = [0]
    num_steps = 100 #hientt change from 1000 to 100 2022.03.23
    # input_img = torch.randn(bg_resize.shape).to(gpu_id)

    x_src = torch.from_numpy(x_src).unsqueeze(0).transpose(1,3).transpose(2,3).float().to(gpu_id)
    x_mask = torch.from_numpy(x_mask.squeeze()).unsqueeze(0).unsqueeze(0).float().to(gpu_id)
    X_after = torch.from_numpy(X).float().to(gpu_id)
    optimizer = get_input_optimizer(X_after)
    X_after = X_after.transpose(0,3).transpose(1,2).transpose(2,3)
    Y = np.zeros(X.shape[:3])
    while run[0] <= num_steps:
        def closure():
            # blend_obj = blended_im
            # blend_obj = blended_im[:,:,int((right-left)*0.5), int(y_start-source_img.shape[3]*0.5):int(y_start+source_img.shape[3]*0.5)]
            source_object_features = vgg(mean_shift(x_src*x_mask))
            blend_object_features = vgg(mean_shift(X_after*x_mask))
    ...
    print(run[0])
    X_after = X_after.transpose(1,3).transpose(1,2).cpu().data.numpy()[0]
    # for i in range(3):
    #     Xdct = dct2(X[:, :, i, 0])
    #     Ydct = (dct2(L[:, :, i]) + color_weight * Xdct) / param 
    #     Y[:, :, i] = idct2(Ydct)
...
def run_gp_editing(src_im, dst_im, mask_im, gan_im, color_weight, sigma, gradient_kernel='normal'):
    ...
    gan_im = gaussian_poisson_editing_loss(feature, src_im, mask_im, param_l, param_g, color_weight=color_weight, content_weight=1, eps=1e-12, gpu_id=0)
...
def gp_gan_content_loss(obj, bg, mask, G, image_size, gpu, color_weight=1, sigma=0.5, gradient_kernel='normal', smooth_sigma=1,
